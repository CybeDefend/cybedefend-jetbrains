...
class ChatBotService(private val apiService: ApiService) {
     ...
     */
    suspend fun continueConversation(conversationId: String, request: AddMessageConversationRequestDto): String =
    ...
    /**
     * Streams SSE updates for a conversation.
     * Calls onDelta for each delta payload.
     * onComplete when stream ends, onError on failure.
     */
    suspend fun streamConversation(
        ...
        val request = Request.Builder()
            .url(url)
            .addHeader("X-API-Key", apiService.authService.getApiKey().orEmpty())
            .build()
            ...
            while (!source.exhausted()) {
                ...
                if (line.isBlank()) {
                    ...
                    when (obj.get("type").asString) {
                        ...
                        "error" -> throw IOException(obj.getAsJsonObject("payload").get("message").asString)
                        else -> Unit
                    }
