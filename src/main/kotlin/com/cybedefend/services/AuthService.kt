...
class AuthService(private val project: Project) {
    ...
    @Volatile
    private var memoryCachedApiKey: String? = null

    private val credentialAttributes = CredentialAttributes(
        generateServiceName("CybeDefend", "APIKey")
    )
    ...
    suspend fun ensureProjectConfigurationIsSet(api: ApiService): ProjectConfig? {
        ...
        val currentProjectRoot = project.basePath ?: return run {
        ...
        getCurrentWorkspaceProjectId(currentProjectRoot)?.let { storedProjectId ->
            ...
            LOG.debug("Found stored project ID: $storedProjectId and org ID: $storedOrgId for root: $currentProjectRoot")
            return ProjectConfig(apiKey, storedProjectId, currentProjectRoot, storedOrgId)
        }
        LOG.debug("No stored project ID found for root: $currentProjectRoot. Proceeding with full configuration flow.")

        val selectedOrgId = selectOrganization(api) ?: return run {
    ...
    fun storeWorkspaceProjectId(id: String) {
        val currentProjectRoot = project.basePath ?: return run {
            LOG.warn("Cannot store workspace project ID, project base path is null.")
        }
    ...
    private fun setWorkspaceProjectId(root: String, projectId: String, orgId: String) {
        LOG.debug("Setting workspace project ID for root '$root' to '$projectId' and orgId to '$orgId'")
        properties.setValue(PROJECT_ID_KEY_PREFIX + root, projectId)
        properties.setValue(ORG_ID_KEY_PREFIX + root, orgId)
    }

    private suspend fun quickInfo(message: String) = withContext(Dispatchers.Main) {
        LOG.debug("Displaying quick info: $message")
        Messages.showInfoMessage(project, message, "CybeDefend Info")
    }

    private suspend fun quickError(message: String) = withContext(Dispatchers.Main) {
    ...
    private suspend fun selectOrganization(api: ApiService): String? {
        ...
        return withContext(Dispatchers.Default) {
            val orgDtos: List<OrganizationInformationsResponseDto> = try {
            ...
            if (chosenOrg != null) {
                ...
                LOG.warn("Selected organization name '$chosenName' not found in the list. This shouldn't happen.")
            }
            chosenOrg?.id
        }
    }
    ...
    private suspend fun tryAutoLinkRepo(api: ApiService, orgId: String, root: String, repoName: String, apiKey: String): ProjectConfig? {
        LOG.debug("Attempting to auto-link repo '$repoName' in org '$orgId' at root '$root'.")
        try {
            val projectsDto = api.getProjectsOrganization(orgId, pageSize = 100, page = 1) // Suspend call
            val matchingProject = projectsDto.projects.firstOrNull { it.name.equals(repoName, ignoreCase = true) }

            if (matchingProject != null) {
                LOG.info("Auto-linking to existing project '${matchingProject.name}' (ID: ${matchingProject.projectId})")
    ...
    private suspend fun fallbackToListProjects(apiKey: String, orgId: String, root: String, api: ApiService): ProjectConfig? {
        LOG.debug("Fallback: Listing projects for manual selection/creation for org '$orgId' at root '$root'.")

        val teamId = selectTeam(api, orgId) ?: return run {
        ...
        if (existingProjectConfig != null) {
            LOG.debug("Selected existing project: ${existingProjectConfig.projectId} during fallback.")
            return existingProjectConfig
        }
        LOG.debug("No existing project selected or user opted to create new. Proceeding to create new project flow.")
        return createNewProject(apiKey, orgId, root, api, teamId)
    }

    private suspend fun selectTeam(api: ApiService, orgId: String): String? = withContext(Dispatchers.Default) {
        LOG.debug("Attempting to select team for organization ID: $orgId")
        val teamDtos: List<TeamInformationsResponseDto> = try {
        ...
        val chosenName = withContext(Dispatchers.Main) {
            Messages.showEditableChooseDialog("Select a team:", "Team Selection", Messages.getQuestionIcon(), teamNames, teamNames.first(), null)
        }
    ...
    private suspend fun selectExistingProject(api: ApiService, orgId: String, teamId: String, rootPath: String, apiKey: String): ProjectConfig? = withContext(Dispatchers.Default) {
        LOG.debug("Attempting to select existing project for org ID '$orgId', team ID '$teamId', root '$rootPath'.")

        val projectsDto: PaginatedProjectsAllInformationsResponseDto = try {
            LOG.debug("Fetching projects for org $orgId (team filter might be implicit or applied if API supports it, page 1, size 100).")
        ...
        // Placeholder: If ProjectsResponseDto.ProjectShort doesn't have teamId, this filtering won't work.
        // Actual filtering might be needed if API doesn't support teamId for project listing or if DTOs need adjustment.
        val teamProjects = projectsDto.projects // .filter { it.teamId == teamId } // Assuming ProjectShort has teamId

        val projectNames = teamProjects.map { it.name }.toTypedArray()
        val choices = projectNames + "[Create New Project]"

        LOG.debug("Prompting user to select from ${projectNames.size} existing projects or create new.")
        val choice = withContext(Dispatchers.Main) {
            Messages.showEditableChooseDialog(
                ...
                Messages.getQuestionIcon(),
                choices,
                if (choices.size == 1) choices.first() else (projectNames.firstOrNull() ?: choices.first()),
                null)
        ...
        if (selectedProject != null) {
            LOG.debug("User selected existing project: ID ${selectedProject.projectId}, Name ${selectedProject.name}")
            setWorkspaceProjectId(rootPath, selectedProject.projectId, orgId)
            quickInfo("Linked to existing project: ${selectedProject.name}")
            return@withContext ProjectConfig(apiKey, selectedProject.projectId, rootPath, orgId)
        }

        LOG.warn("Selected project name '$choice' not found in the list. This shouldn't happen if not creating new.")
        return@withContext null
    }

    private suspend fun createNewProject(apiKey: String, orgId: String, rootPath: String, api: ApiService, teamId: String): ProjectConfig? = withContext(Dispatchers.Default) {
        LOG.debug("Attempting to create new project for org ID '$orgId', team ID '$teamId', root '$rootPath'.")

        val projectNameFromInput = withContext(Dispatchers.Main) {
            Messages.showInputDialog(project, "Enter new project name:", "Create New Project", Messages.getQuestionIcon())
        }
        ...
        try {
            LOG.debug("Calling API to create project '$projectNameFromInput' for team '$teamId'.")
            val newProjectDto: ProjectInformationsResponseDto = api.createProject(teamId, projectNameFromInput) // Suspend call
            LOG.info("Project '${newProjectDto.name}' (ID: ${newProjectDto.projectId}) created successfully via API.")
            setWorkspaceProjectId(rootPath, newProjectDto.projectId, orgId)
            quickInfo("Project '${newProjectDto.name}' created and linked.")
            return@withContext ProjectConfig(apiKey, newProjectDto.projectId, rootPath, orgId)
        } catch (e: Exception) {
